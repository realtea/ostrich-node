...
fn send_text_backpressure() {
    // flexi_logger::Logger::with_str( "send_text_backpressure=trace, async_progress=trace, tungstenite=warn,
    // tokio_tungstenite=warn, ws_stream_tungstenite=warn, tokio=warn" ).start().expect( "flexi_logger");
...
async fn client(send_text: impl Future, client_read: impl Future, steps: Progress<Step>, cs: Endpoint) {
    let conf = WebSocketConfig {
        max_send_queue: Some(1),
        max_message_size: None,
        max_frame_size: None,
