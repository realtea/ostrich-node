...
use futures::{channel::oneshot, future::join, SinkExt, StreamExt};
use log::*;
...
//
async fn partial() {
    // flexi_logger::Logger::with_str( "futures_codec=trace, tungstenite=trace, ws_stream_tungstenite=trace, tokio=warn"
    // ).start().expect( "flexi_logger");
