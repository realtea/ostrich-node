...
//
async fn send_text() {
    // flexi_logger::Logger::with_str( "ping_pong=trace, tungstenite=trace, tokio_tungstenite=trace,
    // ws_stream_tungstenite=trace, tokio=warn" ).start().expect( "flexi_logger");
    ...
    let server = async {
        ...
        let s =
            accept_async(TokioAdapter::new(tcp_stream)).await.expect("Error during the websocket handshake occurred");
        let mut server = WsStream::new(s);
        let mut events = server.observe(ObserveConfig::default()).await.expect("observe server");
    ...
    let client = async {
        ...
        let frame = CloseFrame { code: CloseCode::Unsupported, reason: "Text messages are not supported.".into() };
