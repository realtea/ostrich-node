...
/// Takes a [`WebSocketStream`](async_tungstenite::WebSocketStream) and implements futures 0.3
...
/// - [`WsErr::Protocol`]: The remote made a websocket protocol violation. The connection will be closed gracefully
///   indicating to the remote what went wrong. You can just keep calling `poll_read` until `None` is returned.
/// - tungstenite returned a utf8 error. Pharos will return it as a Tungstenite error. This means the remote send a text
///   message, which is not supported, so the connection will be gracefully closed. You can just keep calling
...
///
/// On writing, eg. `AsyncWrite::*` all errors are fatal. Note that if you get `io::ErrorKind::InvalidData`, it means
/// you send data that exceeds the tungstenite Capacity. Eg. it leads to a message that exceeds the max message size in
/// tungstenite. It's intended that _ws_stream_tungstenite_ protects from this by splitting the data in several
/// messages, but for now there is no way to find out what the max message size is from the underlying WebSocketStream,
/// so that will require changes in the API of _tungstenite_ and _async-tungstenite_, so that will be for a future
/// release.
///
/// When a Protocol error is encountered during writing, it indicates that either _ws_stream_tungstenite_ or
/// _tungstenite_ have a bug so it will panic.
//
...
where S: AsyncRead + AsyncWrite + Send + Unpin
{
    ...
    pub fn new(inner: ATungSocket<S>) -> Self {
        Self { inner: IoStream::new(TungWebSocket::new(inner)) }
    }
}
...
where S: AsyncRead + AsyncWrite + Send + Unpin
{
    ...
    fn poll_read_vectored(
        mut self: Pin<&mut Self>, cx: &mut Context<'_>, bufs: &mut [IoSliceMut<'_>]
    ) -> Poll<io::Result<usize>> {
