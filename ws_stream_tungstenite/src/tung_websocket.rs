...
where S: AsyncRead + AsyncWrite + Send + Unpin
{
    ...
    pub(crate) fn new(inner: ATungSocket<S>) -> Self {
        Self { inner, state: State::empty(), notifier: Notifier::new(), closer: Closer::new() }
    }
...
where S: AsyncRead + AsyncWrite + Send
{
    ...
    ///
    /// - std::io::Error generally mean something went wrong on the underlying transport. Consider these fatal and just
    ///   drop the connection.
    //
...
where S: AsyncRead + AsyncWrite + Send + Unpin
{
    ...
    ///
    /// - [`io::ErrorKind::NotConnected`]: This means that the connection is already closed. You should no longer write
    ///   to it. It is safe to drop the underlying connection when `poll_next` returns None.
    ///
    ///   TODO: if error capacity get's returned, is the socket still usable?
    ///
    /// - [`io::ErrorKind::InvalidData`]: This means that a tungstenite::error::Capacity occurred. This means that you
    ///   send in a buffer bigger than the maximum message size configured on the underlying websocket connection. If
    ///   you did not set it manually, the default for tungstenite is 64MB.
    ///
