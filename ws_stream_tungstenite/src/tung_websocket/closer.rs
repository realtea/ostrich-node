...
impl Closer {
    ...
    //
    pub(super) fn run(
        mut self: Pin<&mut Self>, mut socket: impl Sink<tungstenite::Message, Error = TungErr> + Unpin,
        ph: &mut Notifier, cx: &mut Context<'_>
    ) -> Poll<Result<(), ()>> {
        match &self.state {
            State::Ready => Ok(()).into(),
            ...
            State::Closing(frame) => {
                ...
                match ready {
                    ...
                    Poll::Ready(Ok(())) => {
                        ...
                        //
                        if let Err(e) =
                            Pin::new(&mut socket).as_mut().start_send(TungMessage::Close(Some(frame.clone())))
                        {
                            ph.queue(WsEvent::Error(Arc::new(e.into())));
