...
//
fn send_closeframe() {
    // flexi_logger::Logger::with_str( "send_text_backpressure=trace, tungstenite=trace, tokio_tungstenite=trace,
    ...
    let test = async {
        ...
        let writer = async {
            closer.queue(CloseFrame { code: CloseCode::Unsupported, reason: "tada".into() }).expect("no double close");
        ...
        let reader = async {
            let frame = CloseFrame { code: CloseCode::Unsupported, reason: "tada".into() };
