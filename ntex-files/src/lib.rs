...
use futures::{
    future::{ok, ready, Either, FutureExt, LocalBoxFuture, Ready},
    Future, Stream
};
use hyperx::header::DispositionType;
use mime_guess::from_ext;
use ntex::{
    http::{error::BlockingError, header, Method, Payload, Uri},
    router::{ResourceDef, ResourcePath},
    service::{
    ...
    web::{
        self,
        dev::{WebServiceConfig, WebServiceFactory},
        error::ErrorRenderer,
        guard::Guard,
        FromRequest, HttpRequest, HttpResponse, WebRequest, WebResponse
    }
};
...
type HttpServiceFactory<Err: ErrorRenderer> = BoxServiceFactory<(), WebRequest<Err>, WebResponse, Err::Container, ()>;
            ...
            return match Pin::new(fut).poll(cx) {
                ...
                Poll::Ready(Err(e)) => {
                    let e = match e {
                        ...
                        BlockingError::Canceled => io::Error::new(io::ErrorKind::Other, "Operation is canceled")
                    };
...
fn directory_listing(dir: &Directory, req: &HttpRequest) -> Result<WebResponse, io::Error> {
    ...
    for entry in dir.path.read_dir()? {
        if dir.is_visible(&entry) {
            ...
            if let Ok(metadata) = entry.metadata() {
                if metadata.is_dir() {
                    let _ =
                        write!(body, "<li><a href=\"{}\">{}/</a></li>", encode_file_url!(p), encode_file_name!(entry),);
                } else {
                    let _ =
                        write!(body, "<li><a href=\"{}\">{}</a></li>", encode_file_url!(p), encode_file_name!(entry),);
                }
    ...
    Ok(WebResponse::new(HttpResponse::Ok().content_type("text/html; charset=utf-8").body(html), req.clone()))
}
...
impl<Err: ErrorRenderer> Files<Err> {
    ...
    where for<'r, 's> F: Fn(&'r Directory, &'s HttpRequest) -> Result<WebResponse, io::Error> + 'static {
    ...
    where F: Fn(&mime::Name) -> DispositionType + 'static {
    ...
    where
        ...
        U: ServiceFactory<WebRequest<Err>, Response = WebResponse, Error = Err::Container> + 'static
    {
    ...
    Err::Container: From<FilesError>
{
    fn register(mut self, config: &mut WebServiceConfig<Err>) {
        ...
        let rdef =
            if config.is_root() { ResourceDef::root_prefix(&self.path) } else { ResourceDef::prefix(&self.path) };
...
where Err::Container: From<FilesError>
{
    fn handle_io_error(
        &self, e: io::Error, req: WebRequest<Err>
    ) -> Either<Ready<Result<WebResponse, Err::Container>>, LocalBoxFuture<'static, Result<WebResponse, Err::Container>>>
    {
            ...
            if let Some(ref redir_index) = self.index {
                if self.redirect_to_slash && !req.path().ends_with('/') {
                    ...
                    return Either::Left(ok(req.into_response(
                        HttpResponse::Found().header(header::LOCATION, redirect_to).body("").into_body()
                    )))
                }
                ...
                match x {
                    ...
                    Err(e) => Either::Left(ok(WebResponse::from_err::<Err, _>(FilesError::from(e), req)))
                }
            } else {
                Either::Left(ok(WebResponse::from_err::<Err, _>(FilesError::IsDirectory, req.into_parts().0)))
            }
        } else {
            match NamedFile::open(path) {
                Ok(mut named_file) => {
...
mod tests {
    ...
    async fn test_if_modified_since_without_if_none_match() {
        ...
        let since = hyperx::header::HttpDate::from(SystemTime::now().add(Duration::from_secs(60)));

        let req = TestRequest::default().header(http::header::IF_MODIFIED_SINCE, since.to_string()).to_http_request();
    ...
    async fn test_named_file_text() {
        ...
        assert_eq!(resp.headers().get(http::header::CONTENT_DISPOSITION).unwrap(), "inline; filename=\"Cargo.toml\"");
    }
    ...
    async fn test_named_file_image() {
        ...
        assert_eq!(resp.headers().get(http::header::CONTENT_DISPOSITION).unwrap(), "inline; filename=\"test.png\"");
    }
    ...
    async fn test_named_file_image_attachment() {
        ...
        let resp = test::respond_to(file, &req).await;
        assert_eq!(resp.headers().get(http::header::CONTENT_TYPE).unwrap(), "image/png");
        assert_eq!(resp.headers().get(http::header::CONTENT_DISPOSITION).unwrap(), "attachment; filename=\"test.png\"");
    }
    ...
    async fn test_named_file_status_code_text() {
        let mut file = NamedFile::open("Cargo.toml").unwrap().set_status_code(StatusCode::NOT_FOUND);
        {
            file.file();
            let _f: &File = &file;
        }
    ...
    async fn test_mime_override() {
        ...
        let content_disposition =
            response.headers().get(http::header::CONTENT_DISPOSITION).expect("To have CONTENT_DISPOSITION");
    ...
    async fn test_named_file_ranges_status_code() {
        let mut srv = test::init_service(App::new().service(Files::new("/test", ".").index_file("Cargo.toml"))).await;

        // Valid range header
        let request =
            TestRequest::get().uri("/t%65st/Cargo.toml").header(http::header::RANGE, "bytes=10-20").to_request();
        let response = test::call_service(&mut srv, request).await;
        assert_eq!(response.status(), StatusCode::PARTIAL_CONTENT);

        // Invalid range header
        let request =
            TestRequest::get().uri("/t%65st/Cargo.toml").header(http::header::RANGE, "bytes=1-0").to_request();
        let response = test::call_service(&mut srv, request).await;
    ...
    async fn test_named_file_content_range_headers() {
        let mut srv =
            test::init_service(App::new().service(Files::new("/test", ".").index_file("tests/test.binary"))).await;

        // Valid range header
        let request =
            TestRequest::get().uri("/t%65st/tests/test.binary").header(http::header::RANGE, "bytes=10-20").to_request();

        let response = test::call_service(&mut srv, request).await;
        let contentrange = response.headers().get(http::header::CONTENT_RANGE).unwrap().to_str().unwrap();

        assert_eq!(contentrange, "bytes 10-20/100");

        // Invalid range header
        let request =
            TestRequest::get().uri("/t%65st/tests/test.binary").header(http::header::RANGE, "bytes=10-5").to_request();
        let response = test::call_service(&mut srv, request).await;
    ...
    async fn test_named_file_content_length_headers() {
        ...
        // Invalid range header
        let request =
            TestRequest::get().uri("/t%65st/tests/test.binary").header(http::header::RANGE, "bytes=10-8").to_request();
        let response = test::call_service(&mut srv, request).await;
        assert_eq!(response.status(), StatusCode::RANGE_NOT_SATISFIABLE);
    ...
    async fn test_head_content_length_headers() {
        ...
        // Valid range header
        let request = TestRequest::default().method(Method::HEAD).uri("/t%65st/tests/test.binary").to_request();
        let _response = test::call_service(&mut srv, request).await;
    ...
    async fn test_static_files_with_spaces() {
        let srv = test::init_service(App::new().service(Files::new("/", ".").index_file("Cargo.toml"))).await;
    ...
    async fn test_files_guards() {
        let mut srv = test::init_service(App::new().service(Files::new("/", ".").use_guards(guard::Post()))).await;
    ...
    async fn test_named_file_content_encoding() {
        let mut srv = test::init_service(App::new().wrap(Compress::default()).service(web::resource("/").to(|| {
            async {
                NamedFile::open("Cargo.toml").unwrap().set_content_encoding(http::header::ContentEncoding::Identity)
            }
        })))
        .await;

        let request = TestRequest::get().uri("/").header(http::header::ACCEPT_ENCODING, "gzip").to_request();
    ...
    async fn test_named_file_content_encoding_gzip() {
        let mut srv = test::init_service(App::new().wrap(Compress::default()).service(web::resource("/").to(|| {
            async { NamedFile::open("Cargo.toml").unwrap().set_content_encoding(http::header::ContentEncoding::Gzip) }
        })))
        .await;
        ...
        let res = test::call_service(&mut srv, request).await;
        assert_eq!(res.status(), StatusCode::OK);
        assert_eq!(res.headers().get(http::header::CONTENT_ENCODING).unwrap().to_str().unwrap(), "gzip");
    }
    ...
    async fn test_static_files() {
        let mut srv = test::init_service(App::new().service(Files::new("/", ".").show_files_listing())).await;
    ...
    async fn test_redirect_to_slash_directory() {
        ...
        let mut srv = test::init_service(App::new().service(Files::new("/", ".").redirect_to_slash_directory())).await;
        let req = TestRequest::with_uri("/tests").to_request();
        let resp = test::call_service(&mut srv, req).await;
        assert_eq!(resp.status(), StatusCode::NOT_FOUND);
    ...
    async fn test_path_buf() {
        assert_eq!(PathBufWrp::get_pathbuf("/test/.tt").map(|t| t.0), Err(UriSegmentError::BadStart('.')));
        assert_eq!(PathBufWrp::get_pathbuf("/test/*tt").map(|t| t.0), Err(UriSegmentError::BadStart('*')));
        assert_eq!(PathBufWrp::get_pathbuf("/test/tt:").map(|t| t.0), Err(UriSegmentError::BadEnd(':')));
        assert_eq!(PathBufWrp::get_pathbuf("/test/tt<").map(|t| t.0), Err(UriSegmentError::BadEnd('<')));
        assert_eq!(PathBufWrp::get_pathbuf("/test/tt>").map(|t| t.0), Err(UriSegmentError::BadEnd('>')));
        assert_eq!(PathBufWrp::get_pathbuf("/seg1/seg2/").unwrap().0, PathBuf::from_iter(vec!["seg1", "seg2"]));
        assert_eq!(PathBufWrp::get_pathbuf("/seg1/../seg2/").unwrap().0, PathBuf::from_iter(vec!["seg2"]));
    }
}
