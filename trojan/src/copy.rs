...
use futures_util::{
    future::Future,
    io::{AsyncBufRead as BufRead, AsyncRead as Read, AsyncWrite as Write, BufReader},
    ready
};
use log::trace;
    ...
    W: Write + Unpin + ?Sized
{
    ...
    let future = CopyFuture { reader: BufReader::new(reader), writer, amt: 0, loop_index: 0, label };
    future.await
}
