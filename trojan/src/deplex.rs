...
use crate::util::{poll_write_buf,poll_read_buf};

pub type IoPoll<T> = std::task::Poll<Result<T>>;
    ...
    Out: AsyncRead + AsyncWrite + Unpin,
{
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> IoPoll<()> {
    ...
    T: AsyncRead + Unpin,
{
        ...
        loop {
            ...
            match self.poll_buffer(cx)? {
                ...
                Poll::Ready(Buffered::Eof) => {
                    ...
                    ready!(Pin::new(&mut dst.io).poll_close(cx))?;
    ...
    /// will be read.
    fn poll_buffer(&mut self, cx: &mut Context<'_>) -> IoPoll<Buffered> {
        ...
        if let Some(buf) = self.buf.as_mut() {
            ...
            let sz = ready!(poll_read_buf(Pin::new(&mut self.io), cx,  buf))?;
            // let sz = ready!(Pin::new(&mut self.io).poll_read( cx, buf.buf.as_mut()))?;
    ...
    fn drain_into<U: AsyncWrite + Unpin>(
        ...
        if let Some(buf) = self.buf.as_mut() {
            while buf.has_remaining() {
                ...
                // let n = match Pin::new(&mut dst.io).poll_write(cx, buf.buf.as_mut())? {

                let n = match poll_write_buf(Pin::new(&mut dst.io), cx, buf)? {
...
fn write_zero() -> Error {
    Error::new(ErrorKind::WriteZero, "write zero bytes")
}
