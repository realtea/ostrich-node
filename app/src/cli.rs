...
fn main() -> Result<()> {
    ...
    let matches = Command::new("Ostrich")
        .version("0.1")
        .author("ostrich")
        .subcommand(Command::new("create").about("create an user command").subcommand(Command::new("user")
                    // .about("create a new user")
                    .arg(Arg::new("username")
                        // .about("The username to create")
                        .required(true))))
        .subcommand(Command::new("delete").about("delete an user command").subcommand(Command::new("user")
            // .about("create a new user")
            .arg(Arg::new("username")
                // .about("The username to create")
                .required(true))))
