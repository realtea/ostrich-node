...
use std::{convert::TryFrom, io, net::ToSocketAddrs, sync::Arc};

async fn get(config: Arc<ClientConfig>, domain: &str, port: u16) -> io::Result<(TlsStream<TcpStream>, String)> {
...
fn test_tls12() -> io::Result<()> {
    let fut =
        async {
            ...
            assert!(output.contains("<title>tls-v1-2.badssl.com</title>"), "failed badssl test, output: {}", output);
...
fn test_modern() -> io::Result<()> {
    let fut = async {
        ...
        assert!(output.contains("<title>mozilla-modern.badssl.com</title>"), "failed badssl test, output: {}", output);
